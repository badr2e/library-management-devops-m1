...
class Loan:
    def __init__(self, id=None, book_id=None, member_id=None, loan_date=None, due_date=None, return_date=None, returned=False):
        self.id = id
    ...
    def to_dict(self):
        loan_dict = {
            ...
            'loan_date': self.loan_date.isoformat() if isinstance(self.loan_date, datetime) else self.loan_date,
            'due_date': self.due_date.isoformat() if isinstance(self.due_date, datetime) else self.due_date,
            'returned': self.returned,
        }
        
        if self.return_date:
            loan_dict['return_date'] = self.return_date.isoformat() if isinstance(self.return_date, datetime) else self.return_date
            
        loan_dict['is_overdue'] = datetime.utcnow() > self.due_date if not self.returned else False
